...
function setupLayer(layer){
    ...
    if (layers[layer].achievements){
        setRowColx10(layers[layer].achievements)
...
function setRowColx10(upgrades) {
    if (upgrades.rows && upgrades.cols) return
    let maxRow = 0
    let maxCol = 0
    for (up in upgrades) {
        if (!isNaN(up)) {
            if (Math.floor(up/100) > maxRow) maxRow = Math.floor(up/100)
            if (up%100 > maxCol) maxCol = up%100
        }
    }
    upgrades.rows = maxRow
    upgrades.cols = maxCol
